DROP TABLE IF EXISTS ec_tmp.orders CASCADE;

CREATE TABLE ec_tmp.orders
(
  ...
  purchase_timestamp      TIMESTAMP WITH TIME ZONE, --Shows the purchase timestamp.
  approved_at             TIMESTAMP WITH TIME ZONE, --Shows the payment approval timestamp.
...
SELECT util.add_index('ec_tmp', 'orders', column_names := ARRAY ['order_id', 'customer_id']);
